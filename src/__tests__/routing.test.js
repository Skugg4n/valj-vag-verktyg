...
describe('parseRoute', () => {
  ...
  it('routes /las/:id to the working read view', () => { expect(parseRoute('/las/abc123')).toEqual({ name: 'read', shareId: 'abc123' }) })
  it('/las works on the workshop subdomain too', () => { expect(parseRoute('/las/x', 'verkstaden.olabelin.se')).toEqual({ name: 'read', shareId: 'x' }) })
